...
show03.jsp입니다. <p/>
	...
	//컬렉션 :방의 크기를 자동으로 늘려준다.
		//List :인터페이스를 구현한 객체 : ArraList(싱글쓰레드). Vector(멀티쓰레드)
			//동일한 메소드를 사용
	...
	//루프를 돌아가면서 rs에 담긴 값을 백터에 추가함
		//name,
	if(rs.next()){
		do{
			name.add(rs.getString("name"));
			email.add(rs.getString("email"));
			inputdate.add(rs.getString("inputdat"));
	...
	//레코드의 값이 존재하지 않으면 : falsefor(int i=0; name.size(); i++){
